...
export const useUserTable = (role = '', apiEndpoint = '/api/store-users') => {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      // Jika role kosong (untuk halaman admin/users), kita ingin mengecualikan role ATTENDANT
      // karena dikelola di halaman terpisah
      if (!role) {
        // Di halaman admin/users, jangan tampilkan ATTENDANT
        params.append('excludeRole', 'ATTENDANT');
      } else if (role) {
        // Jika role ditentukan, gunakan role tersebut
        params.append('role', role);
      }

      // Jika role spesifik ditentukan (misalnya untuk halaman pelayan), jangan tambahkan excludeRole
      if (role) {
        params.delete('excludeRole');
      }
