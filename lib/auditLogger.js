...
export async function logAudit({
  ...
  tableName = null,
  entityId = null, // Changed from recordId
  oldValue = null,
  newValue = null,
  ipAddress = null,
  userAgent = null,
  ...
  try {
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entityId, // Changed from recordId
...
export async function logProductCreation(userId, product, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: product.id, // Changed from recordId
    newValue: {
      name: product.name,
...
export async function logProductUpdate(userId, oldProduct, newProduct, storeId, ipAddress, userAgent) {
  ...
  await logAudit({
    ...
    entity: 'Product', // Added entity
    entityId: newProduct.id, // Changed from recordId
    oldValue,
    newValue,
    ipAddress,
    userAgent,
...
export async function logSaleCreation(userId, sale, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: sale.id, // Changed from recordId
    newValue: {
...
export async function logSupplierCreation(userId, supplier, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: supplier.id, // Changed from recordId
    newValue: {
      code: supplier.code,
      name: supplier.name,
...
export async function logCategoryCreation(userId, category, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: category.id, // Changed from recordId
    newValue: {
      name: category.name,
...
export async function logMemberCreation(userId, member, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: member.id, // Changed from recordId
    newValue: {
      code: member.code,
      name: member.name,
      phone: member.phone,
...
export async function logPurchaseCreation(userId, purchase, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: purchase.id, // Changed from recordId
    newValue: {
...
export async function logExpenseCreation(userId, expense, storeId, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: expense.id, // Changed from recordId
    newValue: {
...
export async function logStoreCreation(userId, store, ipAddress, userAgent) {
  await logAudit({
    ...
    entityId: store.id, // Changed from recordId
    newValue: {
      name: store.name,
      code: store.code,
...
export async function logStoreUpdate(userId, oldStore, newStore, ipAddress, userAgent) {
  await logAudit({
    ...
    entity: 'Store', // Added entity
    entityId: newStore.id, // Changed from recordId
    oldValue: {
      name: oldStore.name,
      code: oldStore.code,
...
export async function logUserCreation(userId, user, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entityId: user.id, // Changed from recordId
    newValue: {
      name: user.name,
...
export async function logCreate(userId, entityName, entityData, ipAddress, userAgent, storeId) {
  ...
  await logAudit({
    ...
    entityId: simplifiedData.id || null, // Changed from recordId
...
export async function logDelete(userId, entityName, entityData, ipAddress, userAgent, storeId) {
  ...
  await logAudit({
    ...
    entity: entityName, // Added entity
    entityId: entityData.id || null, // Changed from recordId
    oldValue: entityData,
    ipAddress,
    userAgent,
    storeId,
...
export async function logWarehouseDistribution(userId, distributionData, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entityId: distributionData.id, // Changed from recordId
    newValue: {
...
export async function logWarehousePurchase(userId, purchaseData, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entityId: purchaseData.id, // Changed from recordId
    newValue: {
...
export async function logWarehouseStockAdjustment(userId, adjustmentData, ipAddress, userAgent, storeId) {
  await logAudit({
    ...
    entityId: adjustmentData.id, // Changed from recordId
    newValue: {
