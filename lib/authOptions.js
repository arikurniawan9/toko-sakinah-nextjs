  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const { getLockoutTimeRemaining, recordFailedLoginAttempt, resetLoginAttempts, isLockedOut } = await import('./loginSecurity');

        // Check if user is locked out before proceeding
        const isUserLocked = await isLockedOut(credentials.username);
        if (isUserLocked) {
          const lockoutTime = await getLockoutTimeRemaining(credentials.username);
        ...
        if (!isContextSwitch) {
          ...
          if (!isValid) {
            ...
            const isUserNowLocked = await isLockedOut(credentials.username);
            if (isUserNowLocked) {
              const updatedLockoutTime = await getLockoutTimeRemaining(credentials.username);
              throw new Error(`Terlalu banyak percobaan login gagal. Akun Anda diblokir selama 15 menit.`);
            }
        ...
        if (!isGlobalRole) {
          ...
          if (storeUsers.length > 0) {
            storeAccess = {
              ...
              code: storeUsers[0].store.code, // Include store code
        ...
        if (isContextSwitch && credentials.selectedStoreId && credentials.selectedStoreRole) {
          ...
          if (userStoreAccess) {
            storeAccess = {
              ...
              code: userStoreAccess.store.code, // Include store code
