    ...
    <div className="lg:col-span-2">
      ...
      <div className={`rounded-lg shadow overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className="divide-y max-h-96 overflow-y-auto styled-scrollbar">
          ...
          {!isProductListLoading && products.length > 0 && (
          ...
          {!isProductListLoading && products && Array.isArray(products) && products.map(product => {
            ...
            return (
              <div
                key={product.id}
                className={`flex items-center justify-between p-4 cursor-pointer transition-colors ${product.stock === 0 ? 'opacity-50' : (darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100')} border-l-4 ${product.stock < 5 ? 'border-red-500' : 'border-transparent'}`}
                onClick={() => {
                  if (product.stock === 0) {
                    // Tampilkan notifikasi bahwa stok habis
                    if (showNotification) {
                      showNotification(`Stok produk "${product.name}" habis dan tidak bisa ditambahkan ke keranjang.`, 'error');
                    }
                    return;
                  }
                  addToCart(product);
                ...
                title={product.stock === 0 ? `Stok produk "${product.name}" habis` : `Tambahkan ${product.name} ke keranjang`}
                style={product.stock === 0 ? { pointerEvents: 'none' } : {}}
              >
                <div className="flex-1 min-w-0">
                  <div className={`font-medium truncate ${product.stock === 0 ? (darkMode ? 'text-gray-500' : 'text-gray-400') : (darkMode ? 'text-white' : 'text-gray-900')}`}>{product.name}</div>
                  <div className={`text-sm ${product.stock === 0 ? (darkMode ? 'text-gray-600' : 'text-gray-300') : (darkMode ? 'text-gray-400' : 'text-gray-600')}`}>Kode: {product.productCode}</div>
                </div>
                <div className="text-right ml-4">
                  <div className={`text-sm font-semibold ${product.stock === 0 ? (darkMode ? 'text-gray-500' : 'text-gray-400') : (darkMode ? 'text-purple-400' : 'text-purple-600')}`}>
                    Rp {formatNumber(getTierPrice(product, 1))}
                  </div>
                  <div className={`text-xs mt-1 ${product.stock === 0 ? (darkMode ? 'text-red-400' : 'text-red-600') : (product.stock < 5 ? (darkMode ? 'text-red-400' : 'text-red-600') : (darkMode ? 'text-gray-500' : 'text-gray-500'))}`}>
                    Stok: {product.stock} {product.stock === 0 ? '(Habis)' : (product.stock < 5 && product.stock > 0 ? '(Stok Menipis)' : '')}
                  </div>
                </div>
              </div>
            );
