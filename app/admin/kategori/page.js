...
import { Edit, Trash2, Plus, Download, Upload, X, Save, ArrowLeft, Search, FileText, AlertCircle, CheckCircle, Folder } from 'lucide-react';
...
export default function CategoryManagement() {
  ...
  const handleDelete = async (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus kategori ini?')) {
      try {
        ...
        setError('Terjadi kesalahan saat menghapus kategori: ' + err.message);
  ...
  const handleDeleteMultiple = async () => {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus ${selectedRows.length} kategori?`)) {
      try {
        ...
        fetchCategories();
        setSelectedRows([]);
        setSuccess(`Berhasil menghapus ${selectedRows.length} kategori`);
        setTimeout(() => setSuccess(''), 3000); // Clear success message after 3 seconds
      } catch (err) {
        setError('Terjadi kesalahan saat menghapus kategori: ' + err.message);
        setTimeout(() => setError(''), 5000); // Clear error message after 5 seconds
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Gagal mengimport kategori');
      }
      
      // Refresh data
      fetchCategories();
      
      setSuccess(result.message || `Berhasil mengimport ${result.importedCount || 0} kategori`);
      e.target.value = ''; // Reset file input
        ...
        <Sidebar>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              <h1 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-2xl font-bold`}>Manajemen Kategori</h1>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Tooltip content="Template Kategori">
                  <a 
                    href="/templates/template-kategori.xlsx" 
                    download="template-kategori.xlsx"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <div className="flex flex-col items-center">
                  <Tooltip content={`Hapus ${selectedRows.length} kategori`}>
                    <button
              ...
              <tbody className={`${darkMode ? 'divide-gray-700 bg-gray-800' : 'bg-gray-50'} divide-y ${
                ...
                {categories.length === 0 ? (
                  ...
                  categories.map((category) => (
                    <tr key={category.id} className={`${darkMode ? 'hover:bg-gray-700' : 'hover:bg-pastel-purple-50'}`}>
            ...
            <div className="hidden sm:flex sm:flex-col md:flex md:flex-row md:items-center md:justify-between gap-4 w-full">
              <div>
                <p className={`text-sm ${
                  ...
                  dari <span className="font-medium">{totalCategories}</span> hasil
                </p>
              </div>
              <div>
