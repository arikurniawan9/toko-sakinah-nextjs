...
export default function SupplierManagement() {
  ...
  const { selectedRows, handleSelectAll, handleSelectRow, clearSelection, setSelectedRows } = useTableSelection(suppliers);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Gagal menghapus supplier');
      }
      
      fetchSuppliers();
      if (isMultiple) {
        clearSelection();
        setFormSuccess(`Berhasil menghapus ${itemToDelete.length} supplier`);
      } else {
        setSelectedRows(prev => prev.filter(rowId => rowId !== itemToDelete));
        setFormSuccess('Supplier berhasil dihapus');
      }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      let csvContent = 'Nama,Alamat,Telepon,Email,Tanggal Dibuat,Tanggal Diubah\n';
      data.suppliers.forEach(supplier => {
        const name = `"${supplier.name.split('"').join('""')}"`;
        const address = `"${supplier.address ? supplier.address.split('"').join('""') : ''}"`;
        const phone = `"${supplier.phone ? supplier.phone.split('"').join('""') : ''}"`;
        const email = `"${supplier.email ? supplier.email.split('"').join('""') : ''}"`;
        const createdAt = `"${new Date(supplier.createdAt).toLocaleString()}"`;
        const updatedAt = `"${new Date(supplier.updatedAt).toLocaleString()}"`;
        csvContent += `${name},${address},${phone},${email},${createdAt},${updatedAt}\n`;
      });
      ...
      setFormSuccess('Data supplier berhasil diekspor');
      setTimeout(() => setFormSuccess(''), 3000);
    } catch (err) {
      setTableError('Terjadi kesalahan saat mengekspor supplier: ' + err.message);
      setTimeout(() => setTableError(''), 5000);
    } finally {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Gagal mengimport supplier');
      }
      
      // Refresh data
      fetchSuppliers();
      
      setFormSuccess(result.message || `Berhasil mengimport ${result.importedCount || 0} supplier`);
      e.target.value = ''; // Reset file input
        ...
        <main className="w-full px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className={`rounded-xl shadow-lg ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border`}>
            ...
            <Pagination
              ...
              itemsPerPage={itemsPerPage}
              totalProducts={totalSuppliers} // Renamed from totalProducts to totalSuppliers
              darkMode={darkMode}
            />
          </div>
          ...
          <ConfirmationModal
            ...
            message={`Apakah Anda yakin ingin menghapus ${ 
              Array.isArray(itemToDelete) ? itemToDelete.length + ' supplier' : 'supplier ini'
            }?`}
            darkMode={darkMode}
