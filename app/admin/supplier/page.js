...
export default function SupplierManagement() {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedRows(suppliers.map(supplier => supplier.id));
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus supplier ini?')) {
      try {
        const response = await fetch(`/api/supplier?id=${id}`, {
          method: 'DELETE',
          headers: {
        ...
        setError('Terjadi kesalahan saat menghapus supplier: ' + err.message);
  ...
  const handleDeleteMultiple = async () => {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus ${selectedRows.length} supplier?`)) {
      try {
        ...
        fetchSuppliers();
        setSelectedRows([]);
        setSuccess(`Berhasil menghapus ${selectedRows.length} supplier`);
        setTimeout(() => setSuccess(''), 3000);
      } catch (err) {
        setError('Terjadi kesalahan saat menghapus supplier: ' + err.message);
        setTimeout(() => setError(''), 5000);
    ...
    try {
      ...
      data.suppliers.forEach(supplier => {
        const row = [
          ...
          `"${supplier.phone ? supplier.phone.replace(/"/g, '""') : ''}"`,
      ...
      link.setAttribute('download', `supplier-${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      setSuccess(result.message || `Berhasil mengimport ${result.importedCount || 0} supplier`);
      e.target.value = ''; // Reset file input
        ...
        <Sidebar>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              <h1 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-2xl font-bold`}>Manajemen Supplier</h1>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Tooltip content="Template Supplier">
                  <a 
                    href="/templates/template-supplier.xlsx" 
                    download="template-supplier.xlsx"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <div className="flex flex-col items-center">
                  <Tooltip content={`Hapus ${selectedRows.length} supplier`}>
                    <button
            ...
            <div className="hidden sm:flex sm:flex-col md:flex md:flex-row md:items-center md:justify-between gap-4 w-full">
              <div>
                <p className={`text-sm ${
                  ...
                  dari <span className="font-medium">{totalSuppliers}</span> hasil
                </p>
              </div>
              <div>
