...
export default function AttendantManagement() {
  ...
  // Fetch attendants when dependencies change
  useEffect(() => {
    fetchAttendants();
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedRows(attendants.map(attendant => attendant.id));
    } else {
      setSelectedRows([]);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus pelayan ini?')) {
      try {
        const response = await fetch(`/api/pelayan?id=${id}`, {
          method: 'DELETE',
          headers: {
        ...
        fetchAttendants();
        setSelectedRows(selectedRows.filter(rowId => rowId !== id));
        setSuccess('Pelayan berhasil dihapus');
        setTimeout(() => setSuccess(''), 3000);
      } catch (err) {
        setError('Terjadi kesalahan saat menghapus pelayan: ' + err.message);
  ...
  const handleDeleteMultiple = async () => {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus ${selectedRows.length} pelayan?`)) {
      try {
        ...
        fetchAttendants();
        setSelectedRows([]);
        setSuccess(`Berhasil menghapus ${selectedRows.length} pelayan`);
        setTimeout(() => setSuccess(''), 3000);
      } catch (err) {
        setError('Terjadi kesalahan saat menghapus pelayan: ' + err.message);
        setTimeout(() => setError(''), 5000);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute('download', `pelayan-${new Date().toISOString().slice(0, 10)}.csv`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      
      setSuccess('Data pelayan berhasil diekspor');
      setTimeout(() => setSuccess(''), 3000);
    } catch (err) {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      setSuccess(result.message || `Berhasil mengimport ${result.importedCount || 0} pelayan`);
      e.target.value = ''; // Reset file input
        ...
        <Sidebar>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              <h1 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-2xl font-bold`}>Manajemen Pelayan</h1>
            </div>
  ...
  return (
    <ProtectedRoute requiredRole="ADMIN">
      <Sidebar>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <div className="flex flex-col items-center">
                  <Tooltip content={`Hapus ${selectedRows.length} pelayan`}>
                    <button
                    ...
                    <tr key={attendant.id} className={`${darkMode ? 'hover:bg-gray-700' : 'hover:bg-pastel-purple-50'}`}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input
                          ...
                          onChange={() => handleSelectRow(attendant.id)}
                          className={`h-4 w-4 rounded ${
                            darkMode 
                              ? 'text-pastel-purple-600 bg-gray-700 border-gray-600' 
                              : 'text-pastel-purple-600 focus:ring-pastel-purple-500 border-pastel-purple-300'
              ...
              <div>
                <p className={`text-sm ${
                  ...
                  <span className="font-medium">
                    {Math.min(currentPage * itemsPerPage, totalAttendants)}
                  </span>{' '}
                  dari <span className="font-medium">{totalAttendants}</span> hasil
                </p>
              </div>
              <div>
        ...
        {/* Modal untuk tambah/edit pelayan */}
        {showModal && (
                  ...
                  <div className="sm:flex sm:items-start">
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                      ...
                      <div className="mt-4 w-full">
                        ...
                        <div className={editingAttendant ? "mb-4" : ""}>
