// app/admin/member/page.js (full rewrite to fix syntax error)
'use client';
...
export default function MemberManagement() {
  ...
  const handleDelete = async (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus member ini?')) {
      try {
        ...
        setError('Terjadi kesalahan saat menghapus member: ' + err.message);
  ...
  const handleDeleteMultiple = async () => {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus ${selectedRows.length} member?`)) {
      try {
        ...
        fetchMembers();
        setSelectedRows([]);
        setSuccess(`Berhasil menghapus ${selectedRows.length} member`);
        setTimeout(() => setSuccess(''), 3000);
      } catch (err) {
        setError('Terjadi kesalahan saat menghapus member: ' + err.message);
        setTimeout(() => setError(''), 5000);
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      setSuccess(result.message || `Berhasil mengimport ${result.importedCount || 0} member`);
      e.target.value = ''; // Reset file input
        ...
        <Sidebar>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              <h1 className={`${darkMode ? 'text-white' : 'text-gray-900'} text-2xl font-bold`}>Manajemen Member</h1>
            </div>
  ...
  return (
    <ProtectedRoute requiredRole="ADMIN">
      <Sidebar>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Tooltip content="Template Member">
                  <a 
                    href="/templates/template-member.xlsx" 
                    download="template-member.xlsx"
              ...
              <Tooltip content="Tambah member baru">
                <button
                  onClick={() => {
                    setEditingMember(null);
                    setFormData({ name: '', phone: '', address: '', membershipType: 'silver', discount: 3 });
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end">
                <div className="flex flex-col items-center">
                  <Tooltip content={`Hapus ${selectedRows.length} member`}>
                    <button
            ...
            <div className="hidden sm:flex sm:flex-col md:flex md:flex-row md:items-center md:justify-between gap-4 w-full">
              <div>
                <p className={`text-sm ${
                  ...
                  dari <span className="font-medium">{totalMembers}</span> hasil
                </p>
              </div>
              <div>
        ...
        {/* Modal untuk tambah/edit member */}
        {showModal && (
